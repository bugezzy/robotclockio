    ...
    public function up(): void
    {
        ...
        DB::unprepared(<<<'SQL'
            ...
            begin
                ...
                return query
                select d.user_id, d.full_name, d.team_name, d.work_date,
                       d.session_count, d.open_sessions, d.hours, d.organization_name
                from public.daily_hours d
                where d.work_date >= coalesce(p_from, current_date - 30)
                  and d.work_date <= coalesce(p_to, current_date)
                  and (v_role = 'owner' or d.organization_id = v_org)
        ...
        DB::unprepared(<<<'SQL'
            ...
            begin
                ...
                return query
                select coalesce(d.team_name, '(no team)'),
        ...
        DB::unprepared(<<<'SQL'
            ...
            begin
                ...
                return query
                select p.id, p.card_uid, u.full_name, p.direction, p.punched_at, p.recorded_at, o.name
                ...
                where (p.punched_at at time zone 'America/Chicago')::date
                  ...
                  and (v_role = 'owner' or p.organization_id = v_org)
